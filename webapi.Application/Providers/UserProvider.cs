...
public class UserProvider : IUserProvider
{
    ...
    public (string token, DateTime tokenExpirationDate, int tokenExpirationInMinutes) GenerateToken(Usuario userModel)
    {
        try
        {
            ...
            claims.AddClaim(new Claim("admin", userModel.Rol.Contains("ADMIN") ? "true" : "false"));
            ...
            return new(token, DateTime.UtcNow.AddMinutes(expirationMinutes), expirationMinutes);
        }
        catch (Exception ex)
        {
            System.Runtime.ExceptionServices.ExceptionDispatchInfo.Capture(ex).Throw();
